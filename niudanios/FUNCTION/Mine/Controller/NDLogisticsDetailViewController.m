...
- (void)viewDidLoad {
    ...
    [self.headView.imageViewGoods sd_setImageWithURL:[NSURL URLWithString:ImageUrl(self.modelInfo.gashaponImg)] placeholderImage:[UIImage imageNamed:@"placehold_xx"] completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        NSLog(@"error:%@",error);
    }];
