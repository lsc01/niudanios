...
#pragma mark - 显示时间选择器
+ (instancetype)showDatePickerWithTitle:(NSString *)title dateType:(UIDatePickerMode)type defaultSelValue:(NSString *)defaultSelValue minDateStr:(NSString *)minDateStr maxDateStr:(NSString *)maxDateStr isAutoSelect:(BOOL)isAutoSelect resultBlock:(BRDateResultBlock)resultBlock {
    return [[BRDatePickerView alloc]initWithTitle:title dateType:type defaultSelValue:defaultSelValue minDateStr:(NSString *)minDateStr maxDateStr:(NSString *)maxDateStr isAutoSelect:isAutoSelect resultBlock:resultBlock];
}
...
- (instancetype)initWithTitle:(NSString *)title dateType:(UIDatePickerMode)type defaultSelValue:(NSString *)defaultSelValue minDateStr:(NSString *)minDateStr maxDateStr:(NSString *)maxDateStr isAutoSelect:(BOOL)isAutoSelect resultBlock:(BRDateResultBlock)resultBlock {
    if (self = [super init]) {
        ...
        if (defaultSelValue.length > 0) {
            ...
            _selectDate = [self toDateWithDateString:defaultSelValue];
        } else {
            // 不设置默认日期，就默认选中今天的日期
...
- (void)didSelectValueChanged:(UIDatePicker *)sender {
    ...
    NSLog(@"滚动完成后，执行block回调:%@,%ld", sender.date,sender.minuteInterval);
    // 设置是否开启自动回调
    if (_isAutoSelect) {
        if (_resultBlock) {
...
//    NSLog(@"点击确定按钮后，执行block回调");
    [self dismissWithAnimation:YES];
    if (_resultBlock) {
        _resultBlock(_selectValue,_selectDate);
    }
}
