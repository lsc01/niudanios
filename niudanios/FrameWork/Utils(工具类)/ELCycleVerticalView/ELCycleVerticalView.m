...
- (void)setViewInfo{
    ...
    if(_dataSource.count > 1){
        if (_direction == ELCycleVerticalViewScrollDirectionDown) {
            model = _dataSource[(_indexNow+1)%(_dataSource.count)];
            [self setLabelAttributedText:_tmpTopView andModel:model];
            
        } else {
            model = _dataSource[(_indexNow+1)%(_dataSource.count)];
            [self setLabelAttributedText:_tmpBtmView andModel:model];
